// 110 word cards, 5 words each (550 total words)
const WORD_CARDS = [
...
function createDeck() {
  const shuffled = shuffleArray(WORD_CARDS);
...
function wordsAreIdentical(a, b) {
  const na = normalizeWord(a);
  const nb = normalizeWord(b);
  if (na === nb) return true;
  // Simple plural/suffix check
  const stemA = na.replace(/(?:ies$|es$|s$|ing$|ed$|er$|ion$|ess$|tion$)/, '');
  const stemB = nb.replace(/(?:ies$|es$|s$|ing$|ed$|tion$|ed$|er$|ion$|ess$|tion$)/, '');
  if (stemA.length >= 3 && stemB.length >= 3 && stemA === stemB) return true;
  return false;
}

function cancelDuplicateClues(clues) {
  // clues: array of { playerId, clue, cancelled }
  const result = clues.map(c => ({ ...c, cancelled: false }));
  
  for (let i = 0; i < result.length; i++) {
    if (!result[i].clue) {
      result[i].cancelled = true;
      continue;
    }
    for (let j = i + 1; j < result.length; j++) {
      if (!result[j].clue) continue;
      if (wordsAreIdentical(result[i].clue, result[j].clue)) {
        result[i].cancelled = true;
        result[j].cancelled = true;
      }
    }
  }
  return result;
...
function getScoreMessage(score) {
  if (score === 13) return "Perfect score! Can you do it again?";
  if (score === 12) return "Incredible! Your friends must be impressed!";
  if (score === 11) return "Awesome! That's a score worth celebrating!";
  if (score >= 9) return "Wow, not bad at all!";
  if (score >= 7) return "You're in the average. Can you do better?";
  if (score >= 4) return "That's a good start. Try again!";
  return "Try again, and again, and again.";
}

module.exports = { createDeck, cancelDuplicateClues, wordsAreIdentical, getScoreMessage };
